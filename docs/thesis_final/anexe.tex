...
Aici sunt câteva capturi din aplicație. Casetele de mai jos sunt doar substituenți; se vor înlocui cu
imaginile reale (\code{\textbackslash includegraphics}) după ce le adaug în \code{images/}.

\begin{figure}[!htb]
  \capturaplaceholder{Tabloul de bord (dashboard) după autentificare: un rezumat al activității și
  legăturile către funcționalități.}
...
\begin{figure}[!htb]
  \capturaplaceholder{Sesiunea live: notarea seriilor (greutate, repetări, RPE) și adăugarea de
  exerciții în plus.}
...
\begin{figure}[!htb]
  \capturaplaceholder{Marketplace cu căutare: câmp de căutare, fațete (dificultate, split, grupe
  musculare) și evidențierea potrivirilor (\code{<mark>}).}
  \caption{Marketplace-ul de programe cu căutare full-text și fațete.}
\end{figure}

\begin{figure}[!htb]
  \capturaplaceholder{Modul de coaching: căutarea full-text în istoricul unui client, accesibilă doar
  dacă există o relație activă.}
  \caption{Căutarea în istoricul clientului (mod coaching).}
\end{figure}

\begin{figure}[!htb]
  \capturaplaceholder{Panoul analizorului: scorul pe 100 de puncte, sub-scorurile și avertismentele,
  fiecare cu o sugestie.}
...
Toată schema este creată de migrarea Flyway \code{V1} (28 de tabele), umplută cu date oficiale de
\code{V2} și completată de \code{V3} cu un index pentru concurență. Tabelele, grupate pe
...
\begin{itemize}
  ...
  \item \textbf{Sesiuni (istoric care nu se modifică):} \code{workout\_sessions},
        \code{session\_exercises}, \code{session\_exercise\_muscle\_groups}, \code{session\_routines},
        \code{workout\_sets}.
  \item \textbf{Marketplace:} \code{template\_votes}, \code{template\_saves},
        \code{template\_use\_events}.
  \item \textbf{Tabele pregătite pentru extinderi (nefolosite încă de cod):}
        \code{coach\_session\_comments}, \code{coach\_template\_assignments},
        \code{template\_analysis\_results}, \code{outbox\_events}.
\end{itemize}

Cele mai importante constrângeri pentru integritate sunt: regula că un program public trebuie să aibă
o dată de publicare (\code{CHECK ((visibility='PUBLIC' AND published\_at IS NOT NULL) OR
visibility='PRIVATE')}); interdicția de a te antrena singur (\code{CHECK (coach\_user\_id <>
client\_user\_id)}); indexul unic parțial pentru un singur antrenament activ per utilizator; indecșii
unici parțiali de unicitate cu \code{WHERE deleted\_at IS NULL}; și cheile externe care pot fi anulate
(\code{ON DELETE SET NULL}) către datele sursă, puse în pereche cu coloanele de instantaneu.
Definițiile-cheie sunt în Anexa~3.

% ----------------------------------------------------------------------------
\chapter*{Anexa 3 — Listări de cod reprezentative}
\addcontentsline{toc}{chapter}{Anexa 3 — Listări de cod reprezentative}

\noindent\textbf{Listarea 1.} Felul determinist în care lucrează analizorul: fiecare sub-scor pornește
de la maxim și scade o valoare fixă pentru fiecare problemă (extras simplificat din
\code{TemplateAnalyzerService}).
...
\noindent\textbf{Listarea 2.} Ordinea de autorizare (din \code{SecurityConfig}): RBAC pe rute, peste
care coaching-ul adaugă verificarea ReBAC la nivel de serviciu.
\begin{lstlisting}[language=Java]
...
\noindent\textbf{Listarea 3.} Două definiții SQL importante: garda de concurență (V3) și tabela de
outbox pregătită din V1 pentru extinderea cu outbox tranzacțional.
...
Am măsurat la 7 dimensiuni ale setului de date (1k–2M), cu 300 de iterații măsurate (încălzire 30,
buget de timp 15\,s; \emph{n} arată câte eșantioane s-au strâns când bugetul a limitat o celulă).
PostgreSQL: \code{tsvector}+GIN ordonat cu \code{ts\_rank\_cd}, fuzzy cu \code{pg\_trgm}. OpenSearch:
aceleași mapări și interogări ca în producție.
...
\begin{table}[!htb]
  ...
  \bottomrule
  \end{tabular}
  \caption{Latența căutării fuzzy „benhc” (greșeală de tastare, ms): cum se degradează \code{pg\_trgm}
  față de OpenSearch, care rămâne aproape constant.}
\end{table}
...
Toate endpoint-urile, în afară de cele de autentificare și de starea de sănătate, cer un token JWT
valid. Erorile folosesc un format uniform, cu un cod stabil (de exemplu \code{TEMPLATE\_NOT\_FOUND},
\code{ACTIVE\_WORKOUT\_EXISTS}, \code{SEARCH\_UNAVAILABLE}).
...
Tot stack-ul rulează prin Docker Compose (PostgreSQL 16, OpenSearch 2.13 cu un volum persistent și
aplicația). Indicatorul \code{app.search.enabled} pornește sau oprește căutarea.
...
\noindent Rularea cu date demonstrative (istoric din 2020 până azi) se face cu profilul \code{demo},
