...
\begin{itemize}
  \item \textbf{Autentificare:} \code{POST /api/auth/\{register,login,refresh,logout\}},
        \code{GET /api/auth/me}.
  \item \textbf{Planificare:} \code{/api/exercises}, \code{/api/routines}, \code{/api/gyms},
        \code{/api/gyms/\{id\}/equipment}, \code{/api/equipment/\{id\}}.
  \item \textbf{Programe:} \code{/api/templates}, \code{/api/templates/\{id\}},
        \code{POST /api/templates/\{id\}/\{publish,unpublish\}}.
  \item \textbf{Sesiuni:} \code{POST /api/workouts/start}, \code{GET /api/workouts/active},
        \code{POST /api/workouts/\{id\}/\{finish,cancel\}},
        \code{POST /api/session-exercises/\{id\}/sets}, \code{PUT/DELETE /api/sets/\{id\}}.
  \item \textbf{Istoric și analize:} \code{GET /api/history}, \code{GET /api/analytics/overview}.
  \item \textbf{Marketplace:} \code{GET /api/marketplace/templates},
        \code{POST /api/marketplace/templates/\{id\}/\{vote,save,use\}}.
  \item \textbf{Analizor:} \code{GET /api/templates/\{id\}/analysis}.
  \item \textbf{Căutare:} \code{GET /api/search/templates}, \code{GET /api/search/workouts},
        \code{POST /api/admin/search/reindex} (doar \code{ADMIN}).
  \item \textbf{Coaching:} \code{/api/coach/clients/...} (rol \code{COACH} + relație activă),
        \code{/api/coaching/\{invites,coaches\}}.
\end{itemize}
